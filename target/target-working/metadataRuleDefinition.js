...
function buildMetadataRuleDefinition() {
    return Rapture.object().keys({
        schema: Rapture.object().keys({
            name: Rapture.string().register('artifactType', '__artifact'),
            version: Rapture.version(),
        }).required('name', 'version'),
        id: Rapture.string().register('fullArtifactID', '__artifact', (setupContext) => {
            setupContext.require('artifactType');
            setupContext.onRun((runContext, propertyValue, params) => {
                console.log(params);
                return `${params.artifactType}/${propertyValue}`;
            });
        }),
        version: Rapture.version()
